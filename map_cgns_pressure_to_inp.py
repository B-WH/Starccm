...
def run_gui() -> int:
    ...
    frequency_var = tk.StringVar(value="")
    ...
    def run_job() -> None:
        try:
            frequencies = [
                float(item.strip())
                for item in frequency_var.get().split(",")
                if item.strip()
            ] or None
            result = run_mapping(
    ...
    ttk.Label(frame, text="Frequency Hz").grid(row=5, column=0, sticky="w", pady=6)
    ttk.Entry(frame, textvariable=frequency_var, width=24).grid(row=5, column=1, sticky="w", pady=6)
